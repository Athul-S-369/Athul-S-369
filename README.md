<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=240&section=header&text=ATHUL%20S&fontSize=76&fontColor=D4AF37&fontAlignY=42&desc=Software%20Engineer%20%E2%80%94%20AI%20Systems%20%C2%B7%20Full-Stack%20%C2%B7%20ML%20Engineering&descAlignY=63&descColor=8B8FA8&animation=twinkling" width="100%"/>

<div align="center">

<br/>

[![Typing SVG](https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=700&size=17&duration=2500&pause=900&color=D4AF37&center=true&vCenter=true&width=900&lines=Production+AI+Engineer+%C2%B7+ML+Systems+at+Scale;1.1B-param+LLM+Deployed+at+SRM+Hospital+%E2%80%94+Clinical+Use;FastAPI+%C2%B7+React+%C2%B7+PyTorch+%C2%B7+LangChain+%C2%B7+RAG+%C2%B7+Docker;IEEE%2FSpringer+Paper+in+Progress+%E2%80%94+AI+Transport+Simulation;25%2B+Repos+%C2%B7+End-to-End+%C2%B7+Research+to+Production)](https://athuls.vercel.app)

<br/>

[![Portfolio](https://img.shields.io/badge/%E2%86%92%20athuls.vercel.app%20%E2%86%90-D4AF37?style=for-the-badge&logo=vercel&logoColor=0d1117&labelColor=D4AF37)](https://athuls.vercel.app)

<br/>

[![Profile Views](https://komarev.com/ghpvc/?username=Athul-S-369&color=D4AF37&style=flat-square&label=profile+views)](https://github.com/Athul-S-369)
&nbsp;
[![Email](https://img.shields.io/badge/contact.athuls%40gmail.com-0d1117?style=flat-square&logo=gmail&logoColor=D4AF37)](mailto:contact.athuls@gmail.com)
&nbsp;
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0d1117?style=flat-square&logo=linkedin&logoColor=D4AF37)](https://linkedin.com/in/athul-s-010117289)
&nbsp;
[![Location](https://img.shields.io/badge/Chennai%2C%20TN%20%C2%B7%20Open%20to%20Relocation-1a1a2e?style=flat-square&color=1a1a2e)](mailto:contact.athuls@gmail.com)

</div>

<br/>

<img src="https://capsule-render.vercel.app/api?type=rect&color=gradient&customColorList=6,11,20&height=3" width="100%"/>


## `> whoami`

```text
                         " ,                        
                     :: ,l;` !~[l,                  
                  ;: :! "^   !I!?]-_I               
               ,,       ;-?[}?!   ?]?!              
               +^   ^?cqWB@$$@%*dn,  {;             
              i!   :?xh$$$$$$$$$$$bl-1,             
              ;[  [|_x*$$$$$$$$$$$$uI,                 class AthulS:
               {-_h[I{YdW$$$&WkpoMob";                     role    = "Software Engineer — AI Systems & Full-Stack"
               l mk :I, }nvJc(~)Qbp&U "                    school  = "B.Tech AI · SRMIST · CGPA 8.29 | SGPA 9.05 | No Backlogs"
               !~0x` ,1)l~QaoQ|jLm%*mtz                    based   = "Chennai, TN — Native: Trivandrum, Kerala — Open to Relocation"
               ~zZ{n0ZkZ{z$$&$$$@$@M@kO             
               IYM\zhB*|i]wWoWZOWB%$%aX                    shipped = {
               ixbi!vX?   [jxaWnxoB%kbZ                        "MenoCare"   : "1.1B-param LLM · on-premise · SRM Hospital · daily clinical use",
               `?n  _!  +YpMMkZrn#&%Z!:                        "MenoAssist" : "Full-stack clinical app · React · FastAPI · PostgreSQL · pytest",
                 11 ,}[:lvOp8$@@MB%w-                      }
                  }-  ^+xm*&$$$$Wp\f"               
                  ir   ,}YpLo#qn}lj&),`                    researching = "PDE-based AI transport simulation — IEEE/Springer paper"
                  lf           :|m$$Z!ilI;,^               now         = "Building the next thing."
                   "        :}zkB$$%)!_!l!iilI;,`   
   ^,`.                 ^!}r0*B$$$b];!ii!iIllIllII;"
"l:`                   ^?xXZh*8&w|^,-,~il!I;:;;:ii!i
i'     .               "i(czcr1!`":il'-!I!II,l:`i,i!
    .  `.           .  `'      `I:`_,`_II!I!:ll i`!i
    '  ^. .  '.  ". ^^ ';I:::,,;;.;~^"_IIlIll;i ;.l:
```

**Currently &nbsp;→**&nbsp; [![Currently](https://readme-typing-svg.demolab.com?font=JetBrains+Mono&size=13&pause=900&color=4A9EFF&center=false&vCenter=true&width=680&height=22&lines=Refining+MenoCare+for+broader+clinical+deployment;Co-authoring+IEEE%2FSpringer+AI+transport+paper+%E2%80%94+Q3+2026;Open+to+AI%2FML+engineering+roles+%E2%80%94+relocate+anywhere;Building+the+next+production+AI+system...)](https://athuls.vercel.app)

<img src="https://capsule-render.vercel.app/api?type=rect&color=gradient&customColorList=6,11,20&height=3" width="100%"/>

## Shipped to Production

> Two healthcare AI systems at SRM Hospital. Built solo. In active clinical use.

**MenoCare** &nbsp;—&nbsp; 1.1B-param · 4-bit GGUF · sub-2s CPU inference · zero cloud · 50+ language RAG  
**MenoAssist** &nbsp;—&nbsp; React · FastAPI · PostgreSQL · AI multilingual scoring · doctor reports · full pytest coverage

`PyTorch` `LangChain` `RAG` `FAISS` `FastAPI` `React` `PostgreSQL` `Docker` `AWS S3` `MLflow`

<img src="https://capsule-render.vercel.app/api?type=rect&color=gradient&customColorList=6,11,20&height=3" width="100%"/>

## Build Log

| Project | Stack | One Line |
|---|---|---|
| [**Legal-Doc-Demystifier**](https://github.com/Athul-S-369/Legal-Doc-Demystifier) | Python · React · BERT · FAISS · gRPC · Celery · Redis · JWT | Distributed full-stack — semantic search, gRPC microservices, async queues, full CI/CD |
| [**Stockey**](https://github.com/Athul-S-369/Stockey) | Python · WebSockets · Polars · Grafana · OpenTelemetry · Kubernetes | Real-time financial analytics — cloud-native microservices, production observability |
| **AI Transport Simulation** | Python · PyTorch · PDE/LWR · GeoPandas · Plotly | 22% congestion reduction — co-authoring IEEE/Springer paper (Q3 2026) |
| [**Placement_Pulse**](https://github.com/Athul-S-369/Placement_Pulse) | Python · GitHub Actions | Aggregates internships, jobs & hackathons across India — auto-updated daily |
| [**Schema-Forge**](https://github.com/Athul-S-369/Schema-Forge) | Python · SQLAlchemy · Faker | Synthetic data engine — FK-integrity preserved across complex relational schemas |
| [**HumanizeAI**](https://github.com/Athul-S-369/HumanizeAI) | Python · HuggingFace · NLP | Rewrites AI-generated research into natural academic prose, semantic meaning intact |
| [**BulkData_Consolidator**](https://github.com/Athul-S-369/BulkData_Consolidator) | Python · Pandas · OpenPyXL | Automates 240+ CSV/Excel files — extraction, merging, zero manual work |
| [**Chennai Rent Predictor**](https://github.com/Athul-S-369/Chennai-Rent-Predictor) | XGBoost · FastAPI · ONNX · MLflow · Streamlit | R²=0.92 on 6,000+ real records · ONNX export · MLflow tracked |
| [**IMDB Sentiment Analysis**](https://github.com/Athul-S-369/imdb-sentiment-analysis) | PyTorch · HuggingFace · FastAPI | Fine-tuned transformer — 93%+ accuracy · FastAPI inference endpoint |
| **Text2Emotion** | PyTorch · HuggingFace · Streamlit · FastAPI | Multi-class emotion detection · real-time inference |
| [**DevIssueHub**](https://github.com/Athul-S-369/DevIssueHub) | React · Node.js · MongoDB · Express · JWT | Role-based issue tracker · full-stack · clean REST API |

<img src="https://capsule-render.vercel.app/api?type=rect&color=gradient&customColorList=6,11,20&height=3" width="100%"/>

## Arsenal

<div align="center">

<br/>

[![Skills](https://skillicons.dev/icons?i=py,ts,js,cpp,react,nextjs,threejs,fastapi,flask,nodejs,pytorch,tensorflow,docker,kubernetes,aws,postgres,mongodb,redis,github,grafana,linux,vscode&theme=dark&perline=11)](https://athuls.vercel.app)

<br/>

</div>

<img src="https://capsule-render.vercel.app/api?type=rect&color=gradient&customColorList=6,11,20&height=3" width="100%"/>

## By the Numbers

<div align="center">

<br/>

![1.1B Params](https://img.shields.io/badge/1.1B%20param%20LLM-Clinical%20Deployment-D4AF37?style=for-the-badge&labelColor=161b22)
![50+ Languages](https://img.shields.io/badge/50%2B%20Languages-RAG%20Pipeline-D4AF37?style=for-the-badge&labelColor=161b22)
![sub-2s](https://img.shields.io/badge/sub--2s%20Inference-On--Premise%20CPU-D4AF37?style=for-the-badge&labelColor=161b22)
![93%+](https://img.shields.io/badge/93%25%2B%20Accuracy-Transformer%20Classifier-D4AF37?style=for-the-badge&labelColor=161b22)

![R²=0.92](https://img.shields.io/badge/R%C2%B2%20%3D%200.92-XGBoost%20Predictor-D4AF37?style=for-the-badge&labelColor=161b22)
![22%](https://img.shields.io/badge/22%25%20Congestion%20Cut-AI%20Traffic%20Simulation-D4AF37?style=for-the-badge&labelColor=161b22)
![240+](https://img.shields.io/badge/240%2B%20Files-Automated%20Processing-D4AF37?style=for-the-badge&labelColor=161b22)
![25+](https://img.shields.io/badge/25%2B%20Repos-Open%20Source-D4AF37?style=for-the-badge&labelColor=161b22)

<br/><br/>

<img src="https://github-readme-stats.vercel.app/api?username=Athul-S-369&show_icons=true&theme=github_dark&border_color=D4AF37&title_color=D4AF37&icon_color=D4AF37&text_color=C0C0C0&count_private=true&include_all_commits=true" height="170"/>
&nbsp;&nbsp;
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Athul-S-369&layout=compact&theme=github_dark&border_color=D4AF37&title_color=D4AF37&text_color=C0C0C0&langs_count=8" height="170"/>

<br/><br/>

<img src="https://github-profile-trophy.vercel.app/?username=Athul-S-369&theme=darkhub&no-frame=true&margin-w=10&column=6&title=Commits,PullRequest,Stars,Repositories,Followers,MultiLanguage" width="100%"/>

<br/>

</div>

<img src="https://capsule-render.vercel.app/api?type=rect&color=gradient&customColorList=6,11,20&height=3" width="100%"/>

## Track Record

**B.Tech Artificial Intelligence** · SRMIST, Kattankulathur · 2023–2027 · CGPA **8.29** · SGPA **9.05** · No Backlogs

<br/>

| | |
|---|---|
| **Production** | MenoCare & MenoAssist — SRM Hospital · real patients · data-privacy compliant · full solo ownership |
| **Research** | IEEE/Springer on PDE-based AI transport simulation · 22% congestion reduction · Q3 2026 |
| **Leadership** | R&D Head, Hyperloop Club · 12-member cross-functional team · sprint planning · stakeholder delivery |
| **Hackathons** | Full-stack AI/ML products shipped end-to-end in 24–48h · multiple national-level events |
| **DSA** | 100+ problems · LeetCode & HackerRank · DP, graphs, greedy, segment trees |
| **Portfolio** | [athuls.vercel.app](https://athuls.vercel.app) — production projects, research, live demos |

<br/>

<div align="center">

[![Portfolio](https://img.shields.io/badge/%E2%86%92%20athuls.vercel.app%20%E2%86%90%20Live%20Portfolio-D4AF37?style=for-the-badge&logo=vercel&logoColor=0d1117&labelColor=D4AF37)](https://athuls.vercel.app)
&nbsp;
[![LinkedIn](https://img.shields.io/badge/Connect%20on%20LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/athul-s-010117289)
&nbsp;
[![Email](https://img.shields.io/badge/contact.athuls%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:contact.athuls@gmail.com)

</div>

<br/>

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=140&section=footer&animation=twinkling" width="100%"/>
